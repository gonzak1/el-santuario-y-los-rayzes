...
La mesa principal usa el preset `"Table": "Table_RPG"` (que trae paño rojo de fábrica). Se probó `Table_Custom` + `TableURL` con la textura de madera (quedaba más grande, pero no gustó el resultado visual) y se volvió atrás. También se probó un overlay verde plano sobre el paño (sin cambiar la mesa) y tampoco convenció — se descartó.
